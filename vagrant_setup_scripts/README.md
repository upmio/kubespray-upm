...
- 默认 5 VM 资源计划需要 32 GiB guest 内存；新建集群还必须通过 20% 宿主内存预留检查，因此至少需要 40 GiB `MemAvailable`，建议 64 GiB 以上。
...
- 首次 clone 嵌套部署工作区且配置了代理时，可能写入全局 Git proxy；脚本不会自动恢复该设置。
...
# 推荐：启动交互向导。脚本会解释并询问 VM 网络、CNI 和 Cilium/LB 方案
./libvirt_kubespray_setup.sh

# 只预设节点数量，其余网络配置继续由向导询问
./libvirt_kubespray_setup.sh -c 8

# 自动化安全默认：NAT + Calico，并自动确认普通 yes/no 提示
...
| `-h`, `--help` | 显示帮助 |
| `-v`, `--version` | 显示版本 |
| `-y` | 未预置的选项使用安全默认 NAT + Calico，并自动确认普通 yes/no 提示；Bridge 风险确认和已有 VM 菜单仍交互 |
| `-c <3-50>` | 设置 VM 总数；只修改总数，不修改 etcd、control-plane 或 UPM 节点数 |
| `-n nat\|bridge` | 可选预置 VM 网络模式；省略时由向导询问 |
| `-p, --network-plugin calico\|cilium` | 可选预置 Kubernetes CNI；省略时由向导询问 |
| `--cilium-kube-proxy-replacement` | 可选预置：使用 Cilium eBPF 替换 kube-proxy |
| `--cilium-load-balancer` | 可选预置：启用 Cilium LB IPAM 和 L2 Announcement，并自动启用 kube-proxy replacement |
| `--cilium-lb-range START-STOP` | 可选预置 LoadBalancer 地址池；该参数本身会启用 LoadBalancer |
| `--cilium-lb-interface NAME` | 可选预置 L2 Announcement 使用的 guest 网卡，默认 `eth1`；仅在启用 LoadBalancer 时有效 |

当前脚本不支持 `--nodes`、`--memory`、`--cpus`、`--disk-size`、`--k8s-version` 或组件开关。新建集群会从模板生成 `config.rb`；检测到匹配的 libvirt VM/domain（包括关机状态）时会保留安装期配置和资源。高级配置应修改模板，或在生成配置后改用手工 Vagrant 流程。

网络参数的推荐入口是交互向导。CLI 网络参数不会取代向导，而是预先填写对应字段并跳过该问题，适用于重复部署和自动化。

`-y` 不是完全无人值守模式：未指定 `-n`/`-p` 时采用 NAT + Calico；Bridge 网络配置、Bridge 高风险确认和发现已有 VM 后的处理菜单仍会等待交互输入。

需要交互但标准输入不是 TTY 时，脚本会明确失败并提示使用交互终端或补齐可预置参数，避免在 CI、SSH 管道或后台任务中静默等待输入。
...
默认资源档位：`k8s-1` 为 4 CPU/4 GiB，`k8s-2` 为 4 CPU/4 GiB，其余节点为 6 CPU/8 GiB。默认 5 VM 合计 26 vCPU/32 GiB，已验证的 8 VM 方案合计 44 vCPU/56 GiB。新建集群在修改宿主机前检查 guest 内存不超过当前 `MemAvailable` 的 80%，总 vCPU 不超过宿主 CPU 的 3 倍；已有集群重跑保留安装期资源。模板还会为 `k8s-2` 到最后一个节点各添加一块 200 GiB 磁盘，并尝试将所有检测到的非根磁盘加入 `local_vg_dev`。
...
向导会先选择 VM 网络，再选择 Kubernetes CNI；`-n` 和 `-p` 只是这两个步骤的可选预置参数，两者不能混为同一个“网络插件”。
...
在默认向导中选择 `2. Bridge` 即可；`-n bridge` 仅用于提前选中该项，后续物理网卡、VM CIDR、网关和 DNS 仍由向导结合宿主机现场信息收集。

Bridge 向导会解释风险、列出物理网卡，并在宿主机创建 `br0`、把所选网卡作为 bridge slave。当前实现不会把宿主机原有 IP 自动迁移到 `br0`，远程执行可能立即断开网络；只应通过本地控制台操作并事先准备恢复方案。如果所选网卡已有 IP，即使使用 `-y`，仍必须确认继续并再次输入当前 IP。
...
1. Calico 或 Cilium。
2. 选择 Cilium 时，是否替换 kube-proxy。
3. 是否启用 Cilium LoadBalancer。
4. 启用 LoadBalancer 时，填写地址范围和 guest L2 网卡。

需要自动化或重复使用相同答案时，可以预置参数：
...
任何未预置的字段仍由向导继续询问。Cilium 专属参数不能与 Calico 同时使用；`--cilium-lb-interface` 不能在未启用 LoadBalancer 时单独使用。

Cilium L2 LoadBalancer 依赖 kube-proxy replacement；启用 LoadBalancer 时脚本会自动启用 replacement。Kubespray 会创建 `CiliumLoadBalancerIPPool`，随后主脚本校验节点 InternalIP 所在网卡并创建 `CiliumL2AnnouncementPolicy`。脚本会拒绝与网关、VM 地址以及本项目默认 Pod/Service CIDR 重叠的范围；DHCP、其他 IPPool 和物理设备地址仍需部署者自行排除。
...
主脚本通过 Vagrant `ansible.extra_vars` 传递 `kube_owner`、`kube_proxy_remove` 和 Cilium 变量，不会直接编辑 inventory 下的 `group_vars`。如果绕过主脚本直接运行 `ansible-playbook cluster.yml` 或独立执行 Vagrant，需要自行维护配置，并在部署后手工创建 `CiliumL2AnnouncementPolicy`。
...
不要通过简单的 `vagrant provision` 修改已有集群的 VM 数量、NAT/Bridge、Bridge 地址、Calico/Cilium、kube-proxy replacement、LoadBalancer 地址池或 L2 接口。这些都是集群安装级决策；主脚本在检测到匹配的 libvirt VM/domain（包括关机状态）时读取并锁定已有 `config.rb`，检测到参数冲突时在修改宿主机和配置文件之前拒绝执行。只有残留配置但没有 VM/domain 时会重新进入向导。新生成的 Bridge 配置会额外保存宿主物理网卡，且重跑时必须与 active `br0` slave 匹配。
...
这些测试覆盖参数、向导、资源门禁和配置生成，但不能代替真实 libvirt、Cilium 和 LoadBalancer 部署。
...
- `-y` 使用 NAT + Calico 作为未预置项的安全默认，但不能跳过 Bridge 风险确认或已有 VM 处理菜单。
- 主脚本会创建嵌套部署工作区；外层脚本与内层配置不能混用。
- 普通 worker 固定为 6 CPU/8 GiB，当前没有资源档位 CLI；超过宿主容量门禁时需减少节点数或调整模板后重新评审。
